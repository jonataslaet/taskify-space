...
final class SpaceParticipantPageResult {
  SpaceParticipantPageResult({
    required List<SpaceParticipant> content,
    required this.size,
    required this.number,
    required this.totalElements,
    required this.totalPages,
  }) : content = List<SpaceParticipant>.unmodifiable(content);

  factory SpaceParticipantPageResult.fromJson(Map<String, dynamic> json) {
    final rawContent = json['content'];
    if (rawContent is! List<dynamic>) {
      throw const FormatException('Campo content ausente ou inválido.');
    }

    final content = <SpaceParticipant>[];
    for (var index = 0; index < rawContent.length; index += 1) {
      content.add(
        SpaceParticipant.fromJson(
  ...
  final List<SpaceParticipant> content;
  final int size;
  final int number;
